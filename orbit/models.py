...
class OrbitEntryManager(models.Manager):
    ...
    def _exception_group_key(self):
        ...
        This guarantees *no exception is ever hidden* — a fingerprint-less exception (e.g.
        legacy data, or an event recorded outside the request middleware) groups on its own
        id (count 1) instead of being excluded. Portable across SQLite / MySQL / PostgreSQL.
        """
    ...
    def latest_for_groups(self, group_keys):
        ...
        page* only — one bounded lookup per key. Works for both fingerprint keys and the
        id fallback. Portable (no DISTINCT ON / window-function dependency).
        """
        latest = {}
