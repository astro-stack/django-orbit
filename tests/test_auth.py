...
from orbit.extensions import is_dashboard_request_authorized
...
@pytest.mark.django_db
def test_public_authorization_hook_uses_orbit_auth_check(rf, auth_settings):
    """Mounted extensions inherit the dashboard authorization policy."""
    request = rf.get("/orbit/pro/")
    auth_settings.ORBIT_CONFIG["AUTH_CHECK"] = lambda received: received is request

    assert is_dashboard_request_authorized(request) is True

    auth_settings.ORBIT_CONFIG["AUTH_CHECK"] = lambda received: False
    assert is_dashboard_request_authorized(request) is False
