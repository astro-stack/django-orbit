...
@pytest.mark.django_db
def test_demo_homepage_positions_orbit_as_agent_native(client):
...
def test_pagination_htmx_logic(client):
    ...
    for i in range(30):
        OrbitEntry.objects.create(
            type=OrbitEntry.TYPE_LOG, payload={"message": f"PaginationTest {i}"}
        )

    url = reverse("orbit:feed")
    response = client.get(
        url, {"page": 2, "per_page": 25, "type": "log"}, headers={"HX-Request": "true"}
    )
