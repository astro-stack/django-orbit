...
- Added the metadata-only `capture_health` Evidence API resource for safe inspection of storage availability, effective capture flags, and sanitized watcher readiness.
...
- Request-family correlation now uses async-task-local context, preventing concurrent ASGI tasks from associating logs or LLM watcher entries with the wrong request family.
- `get_request_detail` now returns the versioned, metadata-only Evidence API envelope instead of the legacy payload-bearing entry serialization.
...
- Constrained the MCP extra to the supported 1.x API so the MCP server remains
  compatible with `mcp.server.fastmcp`.
- Made feed pagination and grouped exception representatives deterministic when
  multiple entries share a timestamp.
